<template>
  <div class="app-container calendar-list-container" style="padding: 5px; ">
    ...
    <el-button @click="handleToModel" type="primary"  size='small'  icon="el-icon-star-on">{{$t('table.toModel')}}</el-button>
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter"  size='small'  style="width: 200px; " class="filter-item" :placeholder="$t('table.name')" v-model="listQuery.name">
      </el-input>
      <el-input @keyup.enter.native="handleFilter"   size='small' style="width: 200px;" class="filter-item" :placeholder="$t('table.key')" v-model="listQuery.key">
      </el-input>
      <el-button class="filter-item" type="primary"   size='small' v-waves icon="el-icon-search" @click="handleFilter">{{$t('table.search')}}</el-button>
    </div>

    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      style="width: 100%">
      <!--<el-table-column align="center" :label="$t('table.id')" min-width="200px">-->
      ...
      <!--<el-table-column class-name="status-col" :label="$t('table.status')" width="100">-->
        ...
        <!--</template>-->
      <!--</el-table-column>-->
      <el-table-column align="center" :label="$t('table.actions')" min-width="400px" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status!='published'" size="mini" type="success" @click="handleDeploy(scope.row)">{{$t('table.deploy')}}
          </el-button>
...
<script>
import { fetchList, fetchPv, createApp, deleteModel, deployModel, readResource } from '@/api/process/app'
...
export default {
  ...
  methods: {
    ...
    createModel() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createApp(this.modelTemp).then(response => {
            ...
            if (response.success) {
              ...
              window.open(window.location.origin + '/editor', '_blank')
            } else {
              this.$notify({
                title: '失败',
                message: '创建失败',
    ...
    handleEdit(row) {
      window.open(window.location.origin + '/modeler.html?modelId=' + row.models[0] + '&token=' + getToken(), '_blank')
