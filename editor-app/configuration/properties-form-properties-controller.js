...
var KisBpmFormPropertiesCtrl = ['$scope', '$modal', '$timeout', '$translate', function ($scope, $modal, $timeout, $translate) {
  ...
  var opts = {
    template: 'editor-app/configuration/properties/form-properties-popup-new.html?version=' + Date.now(),
    scope: $scope
  };
...
var KisBpmFormPropertiesPopupCtrl = ['$scope', '$http', '$q', '$translate', '$timeout', function ($scope, $http, $q, $translate, $timeout) {
...
var KisBpmFormPropertiesKeyCtrl = ['$scope', '$modal', '$timeout', '$translate', function ($scope, $modal, $timeout, $translate) {

  // Config for the modal window
  var opts = {
    template: 'editor-app/configuration/properties/form-properties-key-popup.html?version=' + Date.now(),
    scope: $scope
  };
...
var KisBpmFormPropertiesKeyPopupCtrl = ['$scope', '$http', '$q', '$translate', '$timeout', function ($scope, $http, $q, $translate, $timeout) {

  if ($scope.property.value !== undefined && $scope.property.value !== null) {
    $scope.formPropertiesKey = angular.copy($scope.property.value);
  }
  else {
    $scope.formPropertiesKey = '';
  }
  ...
  $q.all([idPromise, namePromise, typePromise]).then(function (results) {
    ...
    $http({method: 'GET', url: KISBPM.URL.getUIList()}).success(function (data, status, headers, config) {
      $scope.formKeyData = data.data.rows
      // Config for grid
      ...
      $scope.gridOptions = {
        ...
        columnDefs: [
          ...
          {field: 'lastUpdateTime', displayName: 'lastUpdateTime'}]
      };
      $scope.translationsRetrieved = true;
  ...
  $scope.save = function () {
    if ($scope.selectedProperties.length > 0) {
      $scope.property.value = '';
      $scope.property.value = $scope.selectedProperties[0].id;
    } else {
      $scope.property.value = null;
    }
    $scope.updatePropertyInModel($scope.property);
...
var KisBpmFormPropertiesPopupNewCtrl = ['$scope', '$http', '$q', '$translate', '$timeout', function ($scope, $http, $q, $translate, $timeout) {
  ...
  $q.all([idPromise, namePromise, typePromise]).then(function (results) {
    ...
    // if ($scope.oldFormProperties.length == 0) {
      $http({
        method: 'GET',
        url: KISBPM.URL.getServiceProperty("FigureEightTask")
