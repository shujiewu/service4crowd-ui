...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
  ...
  construct: function (source, modelMetaData, editorId) {
    ...
    if (ORYX.CONFIG.BACKEND_SWITCH) {
      ...
      new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/stencilsets/editor?version=' + Date.now() + '&token=' + ACTIVITI.CONFIG.token, {
        asynchronous: false,
        method: 'get',
  ...
  serviceName: function () {
    return ORYX.Core.StencilSet.getTranslation(this._jsonObject, "serviceName");
  },
...
ORYX.Core.AbstractShape = ORYX.Core.UIObject.extend(
  /** @lends ORYX.Core.AbstractShape.prototype */
  {
     ...
     */
    toJSON: function () {
      var stencilId=this.getStencil().idWithoutNs();
      if(stencilId.indexOf('-')!==-1){
        stencilId=stencilId.substring(0, stencilId.indexOf('-'))
...
*/
  ...
  addShapeObjects: function (shapeObjects, eventHandler) {
    ...
    var addShape = function (shape, parent) {
      ...
      // if(shape.stencil.id==='ServiceTask')
      //   shape.stencil.id='ServiceTask-'+shape.properties.name
      if(shape.stencil.id==='UserTask')
        shape.stencil.id='UserTask-'+shape.properties.servicename
      var stencil = ORYX.Core.StencilSet.stencil(this.getStencil().namespace() + shape.stencil.id);

      // console.log(shape.stencil.id)
      // console.log(stencil)
      // Create a new Shape
