...
angular.module('activitiModeler')
  .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      $rootScope.atomicService = new Map()
      ...
      $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
        ...
        // 不显示的图形
        var ignoreForPaletteDefinition = []//['SequenceFlow', 'MessageFlow', 'Association', 'DataAssociation',  'SendTask'];//'DataStore',
        var quickMenuItems = [];
        var morphRoles = [];
        ...
        for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
          ...
          for (var i = 0; i < data.stencils[stencilIndex].roles.length; i++) {
            ...
            // 赋予stencilItem 最后一个角色，morphRoles添加所有是它的 应该是可以转形状
        ...
        $scope.containmentRules = containmentRules;  // 包含规则
      ...
      $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
        ...
        if (shapes && shapes.length > 0) {
          ...
          $scope.previousSelectedShape = $rootScope.selectedShape;

          // Only do something if another element is selected (Oryx fires this event multiple times)
          if ($rootScope.selectedShape !== undefined && $rootScope.selectedShape.getId() === selectedShape.getId()) {
          ...
          for (var i = 0; i < properties.length; i++) {
            ...
            if (propertyConfig === undefined || propertyConfig === null) {
              ...
              var currentProperty = {
                ...
                'type': property.type(),
                'mode': 'read',
                'hidden': property.isHidden(),//key==='oryx-servicetaskclass'?true:
                'value': selectedShape.properties[key],
                'writable': key === 'oryx-servicename' ? false : true
              };
              ...
              if (propertyConfig.writeModeTemplateUrl !== null && propertyConfig.writeModeTemplateUrl !== null) {
                ...
                if (!currentProperty.writable) {
                  currentProperty.writeModeTemplateUrl = propertyConfig.readModeTemplateUrl + '?version=' + $rootScope.staticIncludeVersion;
                }
              }
          ...
          // if(stencil.id().indexOf('ServiceTask') !== -1)
          // {
          //   for (var i = 0; i < properties.length; i++)
          //   {
          //     var property = properties[i];
          ...
          //     }
          //     if(key === 'oryx-servicetaskfields')
          //     {
          //          var servicetaskfields = $rootScope.selectedItem.properties[i];
          //          if(selectedShape.properties[key] === null || selectedShape.properties[key].length === 0)
          //          {
          //            $http.get(KISBPM.URL.getServiceProperty(stencil.title()))
          ...
          //                  var propertyx={
          //                    expression: '',
          //                    implementation: '121212',
          //                    name: (response.fields)[j].name,
          //                    string: '',
          //                    stringValue: '121212'
          //                  };
          //                  properties.push(propertyx);
          //                }
          //                var field ={
          //                  fields:properties
          //                };
          //                servicetaskfields.value = field;
          //                // console.log(i)
          //                console.log($rootScope.selectedItem)
          //                // console.log(selectedShape.properties[key])
          //                $rootScope.updatePropertyInModel(servicetaskfields);
          //              });
          //          }
          //     }
          //   }
          // }
          // if(stencil.idWithoutNs().indexOf('UserTask') !== -1){
          //   $http({
          //     method: 'GET',
          //     url: KISBPM.URL.getServiceProperty("HumanTask")
          //   }).success(function (data, status, headers, config) {
          //     for (var i = 0; i < stencil.properties().length; i++) {
          //       if()
          //     }
          //
          //
          ...
          //   });
          // }
          //必须是第一次加载&& selectedShape.properties['oryx-servicepara']===''

          // console.log($rootScope.selectedItem)
          if (stencil.idWithoutNs().indexOf('UserTask') !== -1 ) {
            //   console.log($rootScope.atomicService)
              if($rootScope.atomicService.get(stencil.title())===undefined){
                $http.get(KISBPM.URL.getAtomicList(stencil.title())).success(function (response) {
                  if(response.status===200){
                    $rootScope.atomicService.set(stencil.title(),response.data.rows)
                  }
                });
              }
                for (var i = 0; i < stencil.properties().length; i++) {
                  var property = (stencil.properties())[i];
                  var key = property.prefix() + "-" + property.id();
                  var currentProperty = $rootScope.selectedItem.properties[i];
                  if (key === 'oryx-servicename') {
                    currentProperty.value = stencil.title()
                  ...
                  if (key === 'oryx-name'&& (currentProperty.value===undefined||currentProperty.value==='')) {
                    currentProperty.value = stencil.title()
                    $rootScope.updatePropertyInModel(currentProperty);
                    continue;
                  }
          ...
          if (stencil.idWithoutNs().indexOf('ReceiveTask') !== -1) {
            for (var i = 0; i < stencil.properties().length; i++) {
              var property = (stencil.properties())[i];
              var key = property.prefix() + "-" + property.id();
              var currentProperty = $rootScope.selectedItem.properties[i];
          ...
          if (stencil.idWithoutNs().indexOf('ServiceTask') !== -1 && (selectedShape.properties['oryx-servicetaskfields'] === null || selectedShape.properties['oryx-servicetaskfields'] === undefined || selectedShape.properties['oryx-servicetaskfields'].length === 0)) {
            $http.get(KISBPM.URL.getServiceProperty(stencil.title()))
              .success(function (response) {
                ...
                for (var i = 0; i < stencil.properties().length; i++) {
                  ...
                  // if(key === 'oryx-servicetaskclass' && ((stencil.idWithoutNs().indexOf('HumanTask')===-1)&& (stencil.idWithoutNs().indexOf('ResultTask')===-1)))
                  // {
                  //   var servicetaskclass = $rootScope.selectedItem.properties[i];
                  //   servicetaskclass.value = response.class;
                  //   console.log(i)
                  //   $rootScope.updatePropertyInModel(servicetaskclass);
                  //   continue;
                  // }
                  if (key === 'oryx-servicetaskdelegateexpression') {
                    var servicetaskclass = $rootScope.selectedItem.properties[i];
                    servicetaskclass.value = response.delegateclass;
                    $rootScope.updatePropertyInModel(servicetaskclass);
                    continue;
                  }
                  if (key === 'oryx-servicetasktype') {
                    var servicetasktype = $rootScope.selectedItem.properties[i];
                    servicetasktype.value = response.serviceType;
                    $rootScope.updatePropertyInModel(servicetasktype);
                    continue;
                  }
                  ...
                  if (key === 'oryx-servicetaskfields') {
                    ...
                    for (var j = 0; j < response.fields.length; j++) {
                      var propertyx = {
                        expression: '',
                        implementation: (response.fields)[j].value,
                        name: (response.fields)[j].name,
                        string: '',
                        stringValue: '',
                        type: (response.fields)[j].type
                      };
                      properties.push(propertyx);
                    }
                    var field = {
          ...
          if (stencil.idWithoutNs().indexOf('ServiceTask') !== -1 && selectedShape.properties['oryx-servicepara'] === "") {
            //console.log(selectedShape.properties)
            $http.get(KISBPM.URL.getServiceProperty(stencil.title()))
              .success(function (response) {
                console.log(response)
                for (var i = 0; i < stencil.properties().length; i++) {
                  ...
                  if (key === 'oryx-servicepara') {
                    var servicetaskfields = $rootScope.selectedItem.properties[i];
                    var inputPara = []
                    for (var j = 0; j < response.inputPara.length; j++) {
                      var propertyx = {
                        // expression: '',
                        // implementation:(response.fields)[j].value,
                        // name: (response.fields)[j].name,
                        ...
                        // stringValue: '',
                        // type:(response.fields)[j].type
                        scope: (response.inputPara)[j].scope,
                        description: (response.inputPara)[j].description,
                        value: (response.inputPara)[j].value,
                        name: (response.inputPara)[j].name,
                        type: (response.inputPara)[j].type
                      };
                      inputPara.push(propertyx);
                    }
                    var outputPara = []
                    for (var j = 0; j < response.outputPara.length; j++) {
                      var propertyo = {
                        scope: (response.outputPara)[j].scope,
                        description: (response.outputPara)[j].description,
                        value: (response.outputPara)[j].value,
                        name: (response.outputPara)[j].name,
                        type: (response.outputPara)[j].type
                      };
                      outputPara.push(propertyo);
                    }
                    var field = {
                      inputPara: inputPara,
      ...
      if (!$rootScope.stencilInitialized) {
        ...
        KISBPM.eventBus.addListener(KISBPM.eventBus.EVENT_TYPE_PROPERTY_VALUE_CHANGED, function (event) {
          if (event.property && event.property.key) {
            ...
            if (event.property.key === 'oryx-servicename' && $rootScope.selectedItem !== undefined && $rootScope.selectedItem !== null) {
              $rootScope.selectedItem.title = event.newValue;
            }
        ...
        $scope.safeApply(function () {
          ...
          var showSelectShapeDialog = function () {
            ...
            $modal({
              backdrop: false,
              keyboard: true,
              template: 'editor-app/popups/select-service.html?version=' + Date.now()
            });
          };
    ...
    $scope.propertyClicked = function (index) {
      if (!$rootScope.selectedItem.properties[index].hidden) {
        $rootScope.selectedItem.properties[index].mode = "write";
      }
    };

    /* Helper method to retrieve the template url for a property */
    $scope.getPropertyTemplateUrl = function (index) {
      return $rootScope.selectedItem.properties[index].templateUrl;
    };
    $scope.getPropertyReadModeTemplateUrl = function (index) {
      return $rootScope.selectedItem.properties[index].readModeTemplateUrl;
    };
    $scope.getPropertyWriteModeTemplateUrl = function (index) {
      return $rootScope.selectedItem.properties[index].writeModeTemplateUrl;
    };

    /* Method available to all sub controllers (for property controllers) to update the internal Oryx model */
    $rootScope.updatePropertyInModel = function (property, shapeId) {

      var shape = $rootScope.selectedShape;
    ...
    $scope.findStencilItemInGroupName = function (stencilItemName, group) {
      ...
      if (group.groups && group.groups.length > 0) {
        for (var k = 0; k < group.groups.length; k++) {
          item = $scope.findStencilItemInGroupName(stencilItemName, group.groups[k]);
          if (item) {
            return item;
          }
        }
    ...
    $scope.getStencilItemByName = function (stencilItemName) {
      for (var i = 0; i < $scope.stencilItemGroups.length; i++) {
        var element = $scope.stencilItemGroups[i];

        // Real group
        if (element.items !== null && element.items !== undefined) {
          var item = $scope.findStencilItemInGroupName(stencilItemName, element);
          if (item) {
            return item;
          }
        } else { // Root stencil item
          if (element.name === stencilItemName) {
            return element;
          }
        }
      }
    ...
    $scope.dropCallback = function (event, ui) {
      ...
      if ($scope.dragCanContain) {
        ...
        for (var i = 0; i < stencilSets.length; i++) {
          ...
          var nodes = stencilSet.nodes();
          for (var j = 0; j < nodes.length; j++) {
            if (nodes[j].idWithoutNs() === ui.draggable[0].id)   //这里修改过
            {
              containedStencil = nodes[j];
              break;
            }
          ...
          if (!containedStencil) {
            ...
            for (var j = 0; j < edges.length; j++) {
              if (edges[j].idWithoutNs() === ui.draggable[0].id)  //这里还需要修改
              {
        ...
        if ($scope.quickMenu) {
          ...
          if (shapes && shapes.length == 1) {
            ...
            var option = {};
            option.type = currentSelectedShape.getStencil().namespace() + ui.draggable[0].id;//;containedStencil.idWithoutNs() //ui.draggable[0].id;  这里修改过
