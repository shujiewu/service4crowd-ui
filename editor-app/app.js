...
'use strict';
// activitiModeler.factory('UserInterceptor', ["$q","$rootScope",function ($q,$rootScope) {
//   return {
...
//   };
// }]);
// .factory('httpInterceptor', ['$rootScope', '$q', '$injector',function($rootScope,$q, $injector) {
//   var httpInterceptor = {
//     'responseError' : function(response) {
//       return $q.reject(response);
//     },
...
activitiModeler.factory('httpInterceptor', ['$rootScope', '$q', '$injector',function($rootScope,$q, $injector) {
  var httpInterceptor = {
    'responseError' : function(response) {
      return $q.reject(response);
    },
...
activitiModeler
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
           ...
           * 这段代码的意思应该是说,执行fn这个函数,是否需要调用scope.apply(),如果不需要就直接调用fn,如果需要,那就调用scope.apply(fn)... $$phase 是 angluar 内部使用的状态标志位，用于标识当前是否处于 digest 状态
		       */
            ...
            function fetchModel(modelId, token) {

                var modelUrl = KISBPM.URL.getModel(modelId);
                // console.log('model with id ' + modelId + ' '+modelUrl);
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
                  ...
                  ACTIVITI.CONFIG.token=token;
	                fetchModel(modelId, token);
	                $rootScope.window = {};
